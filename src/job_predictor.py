...
for index,row in df_jobs.iterrows():
    ...
    row['job_text'] = re.sub(r'Data Engineer', '', row['job_text'])    
    row['job_text'] = re.sub(r'Data Scientist', '', row['job_text'])    

# filter for jobs with length greater than 0
df_jobs = df_jobs[df_jobs['job_text'].apply(lambda x: len(x) > 0)]
...
#add length column, filter for job_text greater than 50
df_jobs['job_text_length'] = df_jobs['job_text'].apply(len)
df_jobs = df_jobs[df_jobs['job_text_length'] >= 20]

# =============================================================================
# STEP 3: VISUALIZE DATA (KEYWORD FREQUENCY PER JOB TITLE)
# =============================================================================

titles = ['Data Scientist', 'Data Engineer']

for title in titles:
    
    # filter jobs for job title & count
    df_jobs_copy = df_jobs.copy()
    df_jobs_copy = df_jobs_copy[(df_jobs_copy['job_title'].str.contains(title, case=False))]
    num_jobs = len(df_jobs_copy)     
    
    # Tokenize the article: tokens
    tokens = [word_tokenize(x) for x in df_jobs_copy['job_text']]
    ...
    sns.set()
    fig, ax = plt.subplots(figsize = (6, 10), dpi=250)   
...
# =============================================================================
# STEP 4: PREDICT JOB TITLES BASED ON JOB DESCRIPTION
# =============================================================================

# # =============================================================================
# # MODEL 1: Logistic Regression w/ Bag of Words
# # =============================================================================

# # make series of job text
# job_text_1 = df_jobs['job_text'].copy()
...
#     # join tokenized words back together into long string
#     return " ".join(text)

# # apply preprocess function steps to each job description
# job_text_1 = [text_preprocess(x) for x in job_text_1]
     
# # apply TF-IDF based feature representation
# count_vectorizer = CountVectorizer()
# job_text_vect = count_vectorizer.fit_transform(job_text_1)
# job_text_vect

# # split train/test data 80/20
# X_train, X_test, y_train, y_test = train_test_split(job_text_vect, 
#                                                     df_jobs['label'], 
#                                                     train_size=0.8, 
#                                                     random_state=20)

# # define logistic regression, fit to model                                                    
# scikit_log_reg = LogisticRegression(solver='liblinear', penalty='l1')
...
# mod1_recall = recall_score(y_test, y_pred)
# mod1_precision = precision_score(y_test, y_pred)
# mod1_f1 = f1_score(y_test, y_pred)
...
# # get unique values and array for plot
# classes = np.unique(y_test)
# y_score = scikit_log_reg.decision_function(X_test)
...
#             cbar=False)
# ax.set(xlabel="y-Prediction", ylabel="y-True", xticklabels=classes, 
#        yticklabels=classes, title="Confusion matrix")
# plt.yticks(rotation=0)

# # plot precision-recall curve
# disp = plot_precision_recall_curve(scikit_log_reg, X_test, y_test)
# disp.ax_.set_title('2-class Precision-Recall curve: '
#                    'AP={0:0.2f}'.format(average_precision))


# # # =============================================================================
# # # MODEL 2: Logistic Regression w/ TF-IDF
# # # =============================================================================

# # make series of job text
# job_text_2 = df_jobs['job_text'].copy()
...
# # apply preprocess function steps to each job description
# job_text_2 = [text_preprocess(x) for x in job_text_2]
     
# # apply TF-IDF based feature representation
# tfidf_vectorizer = TfidfVectorizer()
# job_text_vect = tfidf_vectorizer.fit_transform(job_text_2)
# job_text_vect
...
# mod2_recall = recall_score(y_test, y_pred)
# mod2_precision = precision_score(y_test, y_pred)
# mod2_f1 = f1_score(y_test, y_pred)
...
# # =============================================================================
# # MODEL 3: Random Forest with Word2Vec Embeddings
# # =============================================================================

## split dataset
#df_jobs_train, df_jobs_test = model_selection.train_test_split(dtf, test_size=0.3)
...
# # =============================================================================
# # MODEL 4: keras convolutional neural network (CNN)
# # =============================================================================
